...
flags.DEFINE_integer('nepochs', 400, 'Number of epochs to train the model')
...
flags.DEFINE_integer('dim_feedforward', 256, 'Dimension of the feedforward network layer')
flags.DEFINE_integer('nheads', 4, 'Number of heads in the transformer layer')
    ...
    for epoch in range(FLAGS.nepochs):  # loop over the dataset multiple times
        ...
        if epoch % 20 == 0:   
            ...
            if save_model:
                ...
                }, f'checkpoints/{run_name}_epoch_{epoch}.pth')
...
def main(_):
    ...
    if N + val_size > dataset.total_episodes:
        raise ValueError(f"Not enough episodes in dataset for {N} training and {val_size} validation demos. Total: {dataset.total_episodes}")

    # Sample all episodes needed, then split
    episodes = dataset.sample_episodes(n_episodes=N + val_size)
    train_episodes = episodes[:N]
    val_episodes = episodes[N:]
    ...
    if FLAGS.network_type == 'transformer':
        ...
        net = GNNModule(input_dim=embedding_dim, hidden_dim=dim_feedforward, heads=nheads, num_layers=FLAGS.nlayers, use_virtual_node=True, dropedge_p=0.2)
        print("Graph Module")
